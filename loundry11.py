...
<style>
    ...
    button[kind="toolbar"] {display: none;}
    div.stButton > button:first-child {background-color: #2563EB; color: white; font-size: 18px; font-weight: bold; border-radius: 10px; height: 3em; width: 100%; border: none;}
    div.stButton > button:hover {background-color: #1E40AF; color: white;}
    html, body, .stApp { overflow: hidden; height: 100vh; }
</style>
""", unsafe_allow_html=True)

DB_FILE = "jabufi.db"

# ===== 1. FUNGSI BRANKAS SQLITE =====
def init_db():
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS transaksi
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  waktu TEXT, nota TEXT, nama TEXT, wa TEXT,
                  layanan TEXT, kg REAL, total INTEGER)''')
    conn.commit()
    conn.close()
...
init_db() # Panggil pas awal

# ===== TAMPILAN KASIR =====
st.title("🧺 JABUFI LAUNDRY")
st.caption("Kp. Pulo | Data Aman di Brankas")
st.divider()
...
# Tombol 1: Simpan ke SQLITE
if st.button("🧺 SIMPAN & BUAT STRUK"):
    if nama and no_wa:
        nota = f"JF{datetime.now().strftime('%d%m%H%M')}"
...
if not df.empty:
    ...
    if st.button("🗑️ RESET SEMUA RIWAYAT", type="secondary"):
        reset_data()
        st.warning("Data di brankas sudah dikosongkan!")
        st.rerun()
else:
