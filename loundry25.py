...
if not df.empty:
    # BIKIN KOLOM TANGGAL BARU BIAR GAMPANG FILTER
    df['tanggal'] = pd.to_datetime(df['waktu'], format='%d-%m-%Y %H:%M:%S').dt.date
    hari_ini = wib.date()

    total_semua = df['total'].sum()
    # INI KUNCINYA: FILTER BERDASARKAN TANGGAL HARI INI
    df_hari_ini = df[df['tanggal'] == hari_ini]
    total_hari_ini = df_hari_ini['total'].sum()
    jumlah_pelanggan_hari_ini = df_hari_ini.shape[0] # hitung jumlah baris hari ini
